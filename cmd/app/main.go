...
func newRoot() *cobra.Command {
	...
	root.AddCommand(&cobra.Command{Use: "test-dns <domain>", Args: cobra.ExactArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
	...
	root.AddCommand(&cobra.Command{Use: "bot", RunE: func(cmd *cobra.Command, args []string) error {
		return withSyncer(func(ctx context.Context, c *config.Config, s *core.Syncer) error {
	...
	root.AddCommand(&cobra.Command{Use: "add-service <name>", Args: cobra.ExactArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
		return withSyncer(func(ctx context.Context, c *config.Config, sy *core.Syncer) error {
			if e := sy.AddService(ctx, args[0]); e != nil { return e }
	...
	removeCmd := &cobra.Command{Use: "remove-service <name>", Args: cobra.ExactArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
		return withSyncer(func(ctx context.Context, c *config.Config, sy *core.Syncer) error {
			if e := sy.RemoveService(ctx, args[0], removeForce); e != nil { return e }
			out := c.Services[:0]
			for _, x := range c.Services {
				if x != args[0] { out = append(out, x) }
			}
			c.Services = out
			return config.AtomicWrite(cfgPath, c)
		})
	...
	backupCmd := &cobra.Command{Use: "backup <service>", Args: cobra.ExactArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
		return withSyncer(func(ctx context.Context, c *config.Config, sy *core.Syncer) error {
			...
			if backupOut != "" { return os.WriteFile(backupOut, data, 0644) }
	...
	restoreCmd := &cobra.Command{Use: "restore <service>", Args: cobra.ExactArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
		return withSyncer(func(ctx context.Context, c *config.Config, sy *core.Syncer) error {
			var routes []mikrotik.Route
			if restoreFile != "" {
				...
				if err := json.Unmarshal(data, &exp); err != nil { return err }
				routes = exp.Routes
			} else if restoreSnap != "" {
				if err := sy.GetSnapshot(ctx, args[0], restoreSnap, &routes); err != nil { return err }
			} else {
				return fmt.Errorf("specify --from-file or --from-snapshot")
			}
			if !restoreForce { return fmt.Errorf("use --force to confirm restore") }
	...
	snapshotsCmd.AddCommand(&cobra.Command{Use: "list <service>", Args: cobra.ExactArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
		return withSyncer(func(ctx context.Context, c *config.Config, sy *core.Syncer) error {
			snaps, err := sy.ListSnapshots(ctx, args[0])
			if err != nil { return err }
			for _, s := range snaps { fmt.Printf("%s\t%d\n", s.ID, s.Count) }
			return nil
		})
	}})
	snapshotsCmd.AddCommand(&cobra.Command{Use: "delete <service> <id>", Args: cobra.ExactArgs(2), RunE: func(cmd *cobra.Command, args []string) error {
		return withSyncer(func(ctx context.Context, c *config.Config, sy *core.Syncer) error {
			return sy.DeleteSnapshot(ctx, args[0], args[1])
		})
	}})
	var ttlHours int
	cleanupCmd := &cobra.Command{Use: "cleanup <service>", Args: cobra.ExactArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
		return withSyncer(func(ctx context.Context, c *config.Config, sy *core.Syncer) error {
			n, err := sy.CleanupSnapshots(ctx, args[0], time.Duration(ttlHours)*time.Hour)
			if err == nil { fmt.Printf("Deleted %d snapshots\n", n) }
			return err
		})
	}}
	...
	logsCmd.AddCommand(&cobra.Command{Use: "clear", RunE: func(cmd *cobra.Command, args []string) error {
		c, err := loadConfig()
		if err != nil { return err }
		return os.Truncate(c.Logging.File, 0)
	}})
	logsCmd.AddCommand(&cobra.Command{Use: "size", RunE: func(cmd *cobra.Command, args []string) error {
		c, err := loadConfig()
		if err != nil { return err }
	...
	scheduleCmd.AddCommand(&cobra.Command{Use: "list", RunE: func(cmd *cobra.Command, args []string) error {
		c, err := loadConfig()
		if err != nil { return err }
		for _, s := range c.Services { fmt.Printf("%-20s %s\n", s, c.EffectiveSchedule(s)) }
		return nil
	}})
	scheduleCmd.AddCommand(&cobra.Command{Use: "reload", RunE: func(cmd *cobra.Command, args []string) error {
		c, err := loadConfig()
		if err != nil { return err }
	...
	configCmd.AddCommand(&cobra.Command{Use: "validate", RunE: func(cmd *cobra.Command, args []string) error {
		c, err := loadConfig()
		if err != nil { return err }
		return c.Validate()
	}})
	configCmd.AddCommand(&cobra.Command{Use: "reload", RunE: func(cmd *cobra.Command, args []string) error {
		p, err := os.FindProcess(os.Getpid())
		if err == nil { p.Signal(syscall.SIGHUP) }
		fmt.Println("Reload signal sent.")
		return nil
	}})
	configCmd.AddCommand(&cobra.Command{Use: "edit", RunE: func(cmd *cobra.Command, args []string) error {
	...
	configCmd.AddCommand(&cobra.Command{Use: "get <key>", Args: cobra.ExactArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
		c, err := loadConfig()
		if err != nil { return err }
...
func tailLog(path string, n int, follow bool) error {
	...
	buf := make([]string, 0, n)
	for sc.Scan() {
		if len(buf) == n { buf = buf[1:] }
		buf = append(buf, sc.Text())
	}
	for _, l := range buf { fmt.Println(l) }
